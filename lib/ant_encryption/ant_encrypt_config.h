...
#ifndef ANT_ENCRYPT_CONFIG_H__
#define ANT_ENCRYPT_CONFIG_H__
...
typedef void (* ant_encrypt_user_handler_t)(uint8_t channel, ant_encrypt_user_evt_t event);
 ...
 *
 * @return Value returned by @ref ant_crypto_channel_enable (for example, NRF_SUCCESS if
 ...
 * @param[in] p_crypto_config  Pointer to the encryption configuration.
 *
 * @retval 0                                         If the function completed successfully.
 * @retval NRF_ANT_ERROR_INVALID_PARAMETER_PROVIDED  If the channel type is invalid.
 * @retval NRF_ANT_ERROR_CHANNEL_IN_WRONG_STATE      If the stack is not configured for encryption.
