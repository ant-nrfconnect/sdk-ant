...
void ant_bsc_simulator_one_iteration(ant_bsc_simulator_t * p_simulator)
{
    ...
    uint32_t current_speed_pulse_interval;
    uint32_t current_cadence_pulse_interval;

    if (computed_speed == 0) {
        current_speed_pulse_interval = computed_speed;
    }
    else {
        current_speed_pulse_interval = MM_TO_METERS((WHEEL_CIRCUMFERENCE * 1024u) / computed_speed);
    }

    if (computed_cadence == 0) {
        current_cadence_pulse_interval = computed_cadence;
    }
    else {
        current_cadence_pulse_interval = (60u * 1024u) / computed_cadence;
    }
    ...
    if (current_speed_pulse_interval == 0) {
        new_s_events = current_speed_pulse_interval;
    }
    else {
        new_s_events = p_simulator->_cb.time_since_last_s_evt / current_speed_pulse_interval;
    }
    ...
    if (current_cadence_pulse_interval == 0) {
        new_c_events = current_cadence_pulse_interval;
    }
    else {
        new_c_events = p_simulator->_cb.time_since_last_c_evt / current_cadence_pulse_interval;
    }
